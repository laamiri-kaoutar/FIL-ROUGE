            ...
            <div class="grid lg:grid-cols-2 gap-12 items-center">
                ...
                <div class="text-center lg:text-left">
                    ...
                    <div class="mt-8 max-w-2xl mx-auto lg:mx-0">
                            ...
                            <div class="relative">
                                ...
                                <div class="relative bg-white rounded-lg shadow-sm">
                                    <div class="flex items-center px-4 py-3">
                                        ...
                                        <button type="submit" class="ml-4 px-6 py-2 bg-gradient-to-r from-[#8A4FFF] to-[#FF4F8A] text-white rounded-md font-medium hover:shadow-lg transition-shadow">
                                            Search
                                        </button>
                                    </div>
                                </div>
                        ...
                        <div class="mt-4 flex flex-wrap gap-2 justify-center lg:justify-start">
                            @foreach ($categories as $category)
                                <a href="{{ route('client.services') }}?category={{ urlencode($category) }}" 
                                   class="px-4 py-1.5 rounded-full text-sm font-medium text-gray-700 bg-white shadow-sm hover:shadow transition-shadow">
                                    {{ ucwords($category) }}
                                </a>
                            @endforeach
                        </div>
                    ...
                    <div class="mt-12 flex flex-wrap gap-8 justify-center lg:justify-start text-center lg:text-left">
                        <div>
                            <p class="text-2xl font-bold text-gray-900">{{ number_format($freelancerCount) }}+</p>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mt-12 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
                @foreach ($categories as $index => $category)
                    <a href="{{ route('client.services') }}?category={{ urlencode($category) }}" 
                       class="group relative rounded-lg overflow-hidden shadow-md transition-all duration-300 ease-in-out transform hover:-translate-y-1 hover:shadow-lg">
                        <div class="aspect-w-4 aspect-h-3">
                            <img class="object-cover" 
                                 src="{{ $index % 2 == 0 ? 'https://images.pexels.com/photos/3987020/pexels-photo-3987020.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=2' : 'https://images.pexels.com/photos/4065876/pexels-photo-4065876.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=2' }}" 
                                 alt="{{ ucwords($category) }}">
                        </div>
                        <div class="px-4 py-4 bg-white">
                            <h3 class="text-lg font-semibold text-gray-900">{{ ucwords($category) }}</h3>
                            <p class="mt-2 text-base text-gray-600">
                                Discover services in {{ ucwords($category) }} to bring your ideas to life.
                            </p>
                        </div>
                    </a>
                @endforeach
            ...
            <div class="mt-8 text-center">
                <a href="{{ route('client.services') }}" 
                   class="inline-block px-6 py-3 border border-transparent text-base font-medium rounded-full text-white bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700">
                ...
                @if ($testimonials->isNotEmpty())
                    @foreach ($testimonials as $testimonial)
                        <div class="bg-gray-50 rounded-lg p-6">
                            <p class="text-gray-800 text-lg mb-4">"{{ $testimonial->comment }}"</p>
                            <div class="flex items-center">
                                <img src="https://randomuser.me/api/portraits/{{ $testimonial->user->id % 2 == 0 ? 'men' : 'women' }}/{{ $testimonial->user->id % 100 }}.jpg" 
                                     alt="{{ $testimonial->user->name }}" 
                                     class="w-10 h-10 rounded-full mr-4">
                                <div>
                                    <p class="font-semibold text-gray-900">{{ $testimonial->user->name }}</p>
                                    <p class="text-gray-600 text-sm">Client reviewing {{ $testimonial->service->title }}</p>
                                </div>
                            </div>
                        </div>
                    @endforeach
